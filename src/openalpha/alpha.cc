...
void Alpha::UpdateValid(int di) {
  auto values = dr_.GetData("adv60_t").Column<double>(di - delay_);
...
void Alpha::Calculate(int di) {
  auto groups =
      neutralization_ != kNeutralizationByMarket
          ? dr_.GetData(neutralization_ + "_t").Column<int64_t>(di - delay_)
          : nullptr;
  ...
  auto close0 = close.Column<double>(di);
  auto close1 = close.Column<double>(di + 1);
